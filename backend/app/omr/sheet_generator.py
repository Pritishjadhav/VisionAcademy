...
def generate_sheet_pdf(questions: int, choices: int, title: str) -> bytes:
    ...
    total_pages = (questions + MAX_QUESTIONS_PER_PAGE - 1) // MAX_QUESTIONS_PER_PAGE

    for page_index in range(total_pages):
        first_question = page_index * MAX_QUESTIONS_PER_PAGE + 1
        last_question = min(questions, first_question + MAX_QUESTIONS_PER_PAGE - 1)
        page_questions = last_question - first_question + 1

        pdf.setFont("Helvetica-Bold", 16)
        pdf.drawCentredString(page_width / 2, page_height - 36, clean_title)
        pdf.setFont("Helvetica-Bold", 9)
        pdf.drawCentredString(
            page_width / 2,
            page_height - 52,
            f"Questions {first_question}-{last_question} | Page {page_index + 1} of {total_pages}",
        )
        pdf.setFont("Helvetica", 9)
        pdf.drawString(margin, page_height - 72, "Student name: __________________________________")
        pdf.drawRightString(page_width - margin, page_height - 72, "Roll no: __________________")

        top = page_height - 96
        bottom = 45
        grid_height = top - bottom
        row_height = grid_height / page_questions
        bubbles_left = margin + 32
        ...
        for row in range(page_questions):
            ...
            pdf.drawRightString(bubbles_left - 8, y_center - 3, str(first_question + row))
            for choice in range(choices):
                x_center = bubbles_left + (choice + 0.5) * choice_width
        ...
        pdf.drawCentredString(
            page_width / 2,
            25,
            "Fill one bubble completely for each question. Keep the outer border visible when scanning.",
        )
        pdf.showPage()
