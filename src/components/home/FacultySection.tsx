...
const FacultyPlaceholder = () => (
  <svg viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg" className="w-full h-full absolute inset-0 z-10" preserveAspectRatio="xMidYMax meet">
    {/* Clean neutral background with very soft gradient for subtle depth */}
    <defs>
      <linearGradient id="neutralBg" x1="0%" y1="0%" x2="0%" y2="100%">
        <stop offset="0%" stopColor="#ffffff" />
        <stop offset="100%" stopColor="#f1f5f9" />
      </linearGradient>
    </defs>
    <rect width="200" height="200" fill="url(#neutralBg)" />
    
    {/* Geometric Head */}
    <circle cx="100" cy="75" r="32" fill="#334155" />
    
    {/* Geometric Torso */}
    <path d="M35 220 C 35 140, 70 120, 100 120 C 130 120, 165 140, 165 220 Z" fill="#334155" />
    
    {/* Subtle Outline/Collar Detail for elegant separation */}
    <path d="M85 130 C 95 145, 105 145, 115 130" stroke="#f1f5f9" strokeWidth="4" strokeLinecap="round" fill="none" opacity="0.9" />
  </svg>
);
          ...
          <div className="flex gap-6 py-4 animate-faculty-marquee">
            {scrollItems.map((member, idx) => {
              ...
              return (
                <div
                  key={`${member.name}-${idx}`}
                  className={`w-72 shrink-0 rounded-2xl p-8 border flex flex-col items-center text-center transition-all hover:-translate-y-1 relative overflow-hidden ${isFounder
                ...
                >
                  ...
                  <div className={`w-32 h-32 rounded-full mb-5 flex items-center justify-center border-4 border-white relative z-10 overflow-hidden ${
                    member.image 
                      ? `bg-gradient-to-br ${member.color} shadow-md`
                      : "bg-slate-50 shadow-[0_8px_20px_rgba(0,0,0,0.06)]"
                  }`}>
                    {member.image ? (
                      ...
                      <div className="w-full h-full relative flex items-center justify-center">
                        <FacultyPlaceholder />
                        {/* Soft overlay shadow to blend the placeholder into the circle frame perfectly */}
                        <div className="absolute inset-0 rounded-full shadow-[inset_0_2px_8px_rgba(0,0,0,0.05),inset_0_-4px_12px_rgba(0,0,0,0.02)] pointer-events-none z-20" />
                      </div>
                    )}
                  </div>
                  ...
                  <div className="flex gap-3 relative z-10">
                    <a href="#" className={`w-8 h-8 rounded-full flex items-center justify-center shadow-sm transition-colors ${isFounder
