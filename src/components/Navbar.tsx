            ...
            {navLinks.map((link) => {
              ...
              return (
                <Link
                  key={link.name}
                  href={link.href}
                  className={`relative px-4 py-2 font-medium transition-colors z-10 rounded-full ${
                    active ? "text-brand-orange" : "text-slate-700 hover:text-brand-orange"
                  }`}
                >
                  {active && (
                    <motion.div
            ...
            {user ? (
              <div className="relative">
                <button 
                  ...
                  className="w-10 h-10 rounded-full bg-brand-blue/10 flex items-center justify-center text-brand-blue hover:bg-brand-blue/20 transition-colors border border-brand-blue/20 overflow-hidden"
                >
                  {user.photoURL ? (
            ...
            {user ? (
              <div className="relative">
                <button 
                  ...
                  className="w-9 h-9 rounded-full bg-brand-blue/10 flex items-center justify-center text-brand-blue border border-brand-blue/20 overflow-hidden"
                >
                  {user.photoURL ? (
                    <img src={user.photoURL} alt="Profile" className="w-full h-full object-cover" />
                  ) : (
              ...
              <Link href="/login">
                <Button variant="gradient" size="sm" className="h-9 px-3 text-sm">
                  Login
                </Button>
              </Link>
            )}
