...
export default function AdminStudentReviewPage({ params }: { params: Promise<{ batchId: string, studentId: string, testId: string }> }) {
  ...
  useEffect(() => {
    async function fetchReviewData() {
      ...
      try {
        ...
        if (tSnap.exists()) {
          ...
          router.replace(`/admin/dashboard/batch/${encodeURIComponent(batchName)}/student/${studentId}`);
          return;
        }
        ...
        // Fetch Answers
        const ansQ = query(collection(db, "studentAnswers"), where("testId", "==", testId), where("studentId", "==", studentId));
    ...
    fetchReviewData();
  }, [testId, studentId, batchName, user, router]);
    ...
    <div className="space-y-8 max-w-5xl mx-auto py-8 px-4 sm:px-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div className="flex items-center gap-4">
          <Link href={`/admin/dashboard/batch/${encodeURIComponent(batchName)}/student/${studentId}`}>
            <button className="p-2 bg-white border border-slate-200 rounded-xl hover:bg-slate-50 transition-colors">
              <ArrowLeft size={20} className="text-slate-700" />
            </button>
          </Link>
          <div>
            <h1 className="text-3xl font-bold text-slate-900 line-clamp-1">{test.testName} (Review)</h1>
                ...
                {q.questionType !== 'Integer' && q.options && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    {['A', 'B', 'C', 'D'].map(opt => {
                      ...
                      else if (isOptionCorrect && !isOptionSelected) optClass = "border-green-500 bg-green-50 ring-2 ring-green-500/20"; 
                      ...
                      return (
                        <div key={opt} className={`p-4 rounded-xl border ${optClass}`}>
                          <div className="flex justify-between items-start">
                            ...
                            {isOptionSelected && (
                              <span className="text-xs font-bold px-2 py-1 bg-white border rounded shadow-sm text-slate-600">Student Answer</span>
                            )}
                          </div>
                        </div>
                      )
                  ...
                  <div className={`grid grid-cols-1 ${studentAnswers ? 'sm:grid-cols-2' : ''} gap-4 mb-6`}>
                    {studentAnswers && (
                      <div className="p-4 rounded-xl border border-slate-200 bg-slate-50">
                        <p className="text-sm text-slate-500 mb-1 font-medium">Student Answer</p>
