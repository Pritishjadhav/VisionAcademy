    ...
    async function fetchTest() {
      try {
        ...
        if (testSnap.exists()) {
          ...
          const startDateTime = new Date(`${data.testDate}T${data.startTime}`);
          if (new Date() >= startDateTime) {
            toast.error("Cannot edit a test that has already started");
          ...
          setFormData({
            ...
            marksPerCorrectAnswer: data.marksPerCorrectAnswer || 4,
            marksPerWrongAnswer: data.marksPerWrongAnswer || 1,
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto py-6 px-4">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-slate-900">Edit Test Details</h1>
          <p className="text-slate-500">Update configuration for {formData.testName}</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8 space-y-8">
        ...
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-slate-800 border-b border-slate-100 pb-2">Basic Information</h2>
